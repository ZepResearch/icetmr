...
export async function POST(request) {
  try {
    ...
    const record = await pb.collection("ICETMR_journal_form_submission").create(pbFormData)
    ...
    await resend.emails.send({
      from: "ICETMR |Journal-Submission <info@emergingtrendsconference.com>",
      to: data.email,
      subject:
        "Journal Submission Confirmation - ICETMR  ",
      html: getUserEmailTemplate(data),
    })

    // Send notification email to admin
    await resend.emails.send({
      from: "ICETMR  | Submission <info@emergingtrendsconference.com>",
      to: "info@emergingtrendsconference.com", // Replace with actual admin email
      subject: "New Journal Submission - from ICETMR ",
